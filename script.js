...
function calculate() {
    // Get form values
    var height_inches = parseFloat(document.getElementById("height_inches").value);
    ...
    // Validate inputs
    if (isNaN(height_inches) || isNaN(wrist) || isNaN(gender) || isNaN(body) || isNaN(growth)) {
...
function seven_peices_set(club_max) {
    output_to_screen('Driver Length: ', (Math.round(club_max * 4) / 4).toFixed(2), 'driverlength');
    output_to_screen('Fairway wood length: ', (Math.round((club_max - 1) * 4) / 4).toFixed(2), 'fairwaywoodlength');
    output_to_screen('Hybrid wood length: ', (Math.round((club_max - 2) * 4) / 4).toFixed(2), 'hybridwoodlength');
    output_to_screen('7 Iron length: ', (Math.round(club_max * 0.84 * 4) / 4).toFixed(2), 'ironlength7');
    output_to_screen('8 Iron Length: ', (Math.round(club_max * 0.825 * 4) / 4).toFixed(2), 'ironlength8');
    output_to_screen('Wedge length: ', (Math.round(club_max * 0.79 * 4) / 4).toFixed(2), 'wedgelength');
    output_to_screen('Putter length: ', (Math.round(club_max * 0.78 * 4) / 4).toFixed(2), 'putterlength');
}
